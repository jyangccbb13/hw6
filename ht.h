...
{
    if(tSize >= resizeAlpha_ * table_.size()){
        resize();
    }
    ...
    if (h == npos) 
    {
        throw std::logic_error("No free space for insertion");
    }
...
{
    HashItem* toRemove = internalFind(key);
    if (toRemove != NULL)
    {
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    {
        throw std::logic_error("Max cap reached");
    }
    
    std::vector<HashItem*> oldTable = table_;
    table_ = std::vector<HashItem*>(CAPACITIES[mIndex_], nullptr);
    // Rehash all the non-deleted items from the old table to the new table
...
HASH_INDEX_T HashTable<K,V,Prober,Hash,KEqual>::probe(const KeyType& key) const
{
    ...
    while(Prober::npos != loc)
    {
        ...
        else if(!table_[loc]->deleted && kequal_(table_[loc]->item.first, key) && table_[loc] != NULL) {
            return loc;
        }
